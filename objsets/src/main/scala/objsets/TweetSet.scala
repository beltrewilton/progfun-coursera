...
class Empty extends TweetSet {
  ...
  override def highestTweet(that: Tweet): Tweet = that
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val lr = left.filterAcc(p, right.filterAcc(p, acc))
    if (p(elem)) lr.incl(elem)
    else lr
  }

  override def union(that: TweetSet): TweetSet = right.union(left.union(that.incl(elem)))

  override def mostRetweeted: Tweet = left.highestTweet(right.highestTweet(elem))
  override def highestTweet(that: Tweet): Tweet = {
    val higher: Tweet = mostRetweeted
    if (higher.retweets > that.retweets) higher else that
  }
